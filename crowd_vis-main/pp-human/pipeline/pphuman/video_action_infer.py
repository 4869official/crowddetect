...
from openvino_infer.openvino_video_action_infer import video_predictor
...
class VideoActionRecognizer(object):
    ...
    def __init__(self,
        ...
        if run_mode == "openvino" and 0:
            print(model_dir)
            model_file_path = glob.glob(os.path.join(model_dir, "*.onnx"))[0]
            self.predictor = video_predictor(model_file_path, core)
        else:
    ...
    @classmethod
    def init_with_cfg(cls, args, cfg, core):
    ...
    def predict(self, input):
        ...
        Returns:
            results (dict): 
        '''
        if self.run_mode =="openvino" and 0:
            # preprocess
