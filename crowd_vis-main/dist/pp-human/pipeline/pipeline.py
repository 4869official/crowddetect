...
from dbmodel.models import crowdinfo, warning, address,device
...
camera_ids,camera_idss = getid()

rtsplinks = [
    f"rtsp://{camera_idss[i]}/dss/monitor/param?cameraid={camera_ids[i]}&substream=1" for i in range(len(camera_ids))
]'''
def getidd():
    aid = []
    # 使用all方法获取govinf模型中的所有对象
    objs = device.objects.all()
    for obj in objs:
        # 将每个对象的idcode字段值添加到aid列表中
        aid.append(obj.bh)
    return aid
...
'''rtsplinks = [
    f"rtsp://{camera_idss[0]}/dss/monitor/param?cameraid={i}&substream=1" for i in camerabh
]'''
rtsplinks = [
    "rtsp://13.1.4.60:8319/dss/monitor/param?cameraid=13010701001320109306&substream=1"
]
...
def getinfo(caid):
    # 从govinf模型中查询数据
    try:
        # 使用get方法根据idcode查询，如果找到则返回对象，否则抛出DoesNotExist异常
        obj = device.objects.get(idcode=caid)
        # 获取查询结果中的各个字段值
        qwe = obj.mc  # 通道名称
        asd = obj.jd # 经度
        zxc = obj.wd # 纬度
...
class ImageWindow(QWidget):
    def __init__(self, frame_rgb, humannum, humannum2):
        ...
        imagepathhh = "D:\\apache-tomcat-9.0.52\\webapps\\ROOT\\tupian\\"
        ...
        while (1):
            ...
            if self.modebase["idbased"] or self.modebase["skeletonbased"]:
                ...
                if frame_id % 10 == 0:
                    ...
                    location, jingdu, weidu, classl = getinfo(camera_idss[cameraindex])
                    ...
                    if (len(mot_res['boxes']) > 1 and cont) or classl==2:  # 人数
                        ...
                        if fire == 1:
                            ...
                            files = os.listdir('D:\\apache-tomcat-9.0.52\\webapps\\ROOT\\tupian')

                            img1str = imagepath
                            iiii=location+iii
                            imgth=getinfo(camera_idss[cameraindex])+":8089/tupian/"+time.strftime("%Y%m%d_%H%M%S", time.localtime()) + ".jpg"

                            db_obj1 = address(address=location, shoottime=iiii,
                                              id=random.randint(1, 100000), imagepicpath=imgth, videooopath=imagepath,
                                              state=bostr, num=10001, recording={"1": ppppnum}, banner="fire",
                                              mourn="fire", jd=jingdu, wd=weidu, cameraindex=camera_idss[cameraindex])
                        ...
                        if label == "car crash" and confid >= 0.5:
                            ...
                            img1str = imagepath
                            iiii = location + iii
                            imgth = getip() + ":8089/tupian/" + time.strftime("%Y%m%d_%H%M%S",time.localtime()) + ".jpg"


                            db_obj1 = address(address=location, shoottime=iiii,
                                              id=random.randint(1, 100000), imagepicpath=imgth, videooopath=imagepath,
                                              state=bostr, num=10002, recording={"1": ppppnum}, banner="car crash",
                                              mourn="car crash", jd=jingdu, wd=weidu, cameraindex=camera_idss[cameraindex])
                        ...
                        writer.release()
                        out1 = os.path.join("D:\\apache-tomcat-9.0.52\\webapps\\ROOT\\shipin",
                                            time.strftime("%Y%m%d_%H%M%S", time.localtime()) + ".mp4")
                        out2 = "D:\\apache-tomcat-9.0.52\\webapps\\ROOT\\tupian\\" + str(out1)
                        fourcc = cv2.VideoWriter_fourcc(*'mp4v')
                        # fourcc=-1
              ...
              #9 11 0 7
                        ...
                        try:
                            if setlocal==0:
                                addresses = address.objects.filter(cameraindex=camera_idss[cameraindex])
                            else:
                        ...
                        if lastr:
                            ...
                            if not daycheck:
                                ...
                                if (classl != 1) and (classl != 2):
                                    iiii = location + iii
                                    imgth = getip() + ":8089/tupian/" + time.strftime(
                                        "%Y%m%d_%H%M%S", time.localtime()) + ".jpg"
                                    db_obj1 = address(address=location, shoottime=iiii,
                                                      id=random.randint(1, 100000), imagepicpath=imgth,
                                                      videooopath=out2,
                                                      state=bostr, num=pppnum, recording={"1": ppppnum}, banner=text7,
                                                      mourn=mourn, jd=jingdu, wd=weidu,
                                                      cameraindex=camera_idss[cameraindex])
                                    db_obj1.save()
                        ...
                        if not lastr:
                            ...
                            if (classl != 1) and (classl != 2):
                                ...
                                db_obj1 = address(address=location, shoottime=iiii,
                                                  id=random.randint(1, 100000), imagepicpath=imgth, videooopath=out2,
                                                  state=bostr, num=pppnum, recording={"1": ppppnum}, banner=text7,
                                                  mourn=mourn, jd=jingdu, wd=weidu, cameraindex=camera_idss[cameraindex])
                                '''
                                if text5=="":
                                    numo1=0
                                    numo2=0
